...
# Autosend - Copyright (C) 2010 Cristian Consonni
#
...
def azzera_errori(cfg, res):
	...
	logger.debug("val: %s" %val)
        if val == True:
	    logger.debug("tutto bene")
            continue
        if isinstance(cfg.get(key), dict):
            # Go down one level
            logger.debug("--- giù un livello ---")
            conf = azzera_errori(cfg[key], val)
            logger.debug("conf: %s" %conf)
            cfg[key] = conf
            continue
        logger.debug("tutto male")
        cfg[key] = None
...
def _update_dict(section, key, diz):
  """
  Funzione per riversare tutti i contenuti in un dizionario
  """
  ...
  #print "In update_dict"
  
  #print "(val, key, section) = %s, %s, %s" %(val, key, section)
...
def parse(config_infile, spec_infile):
  ...
  try:
    ...
    print 'Could not read "%s": %s' % (config_infile, e)
    raise IOError
